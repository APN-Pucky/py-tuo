...
extern "C" JNIEXPORT void
    ...
    Java_de_neuwirthinformatik_Alexander_mTUO_TUOIntentService_callMain(
        JNIEnv *env, jobject obj /* this */, jobjectArray stringArray) {
  envv = env;
  objv = obj;
  // from:
  // https://stackoverflow.com/questions/8870174/is-stdcout-usable-in-android-ndk
  // and https://gist.github.com/dzhioev/6127982
  ...
  private:
    ...
    int sync() {
      ...
      if (this->pbase() != this->pptr()) {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "TUO_TUO", "%s", sss);
        jstring jstr = envv->NewStringUTF(sss);
        jclass clazz = envv->FindClass(
            "de/neuwirthinformatik/Alexander/mTUO/TUOIntentService");
        jmethodID messageMe =
            envv->GetMethodID(clazz, "output", "(Ljava/lang/String;)V");
...
extern "C" JNIEXPORT jstring
    ...
    JNICALL
    Java_de_neuwirthinformatik_Alexander_mTUO_TUOIntentService_stringFromJNI(
...
unsigned get_required_cards_before_upgrade(
    std::unordered_map<unsigned, unsigned> &owned_cards,
    const std::vector<const Card *> &card_list,
  ...
  // * use only top-level cards for initial (basic) dominion (Alpha Dominion)
  // and dominion material (Dominion Shard)
  while (!unresolved_cards.empty()) {
    ...
    if ((card->m_set == 1000) && (card->m_rarity <= 2) &&
        (card->is_low_level_card())) {
      continue;
    }
    ...
    if ((fund || (card->m_category != CardCategory::normal)) &&
        (owned_cards[card->m_id] < num_cards[card]) &&
        !card->m_recipe_cards.empty()) {
...
inline unsigned
get_required_cards_before_upgrade(const std::vector<const Card *> &card_list,
                                  std::map<const Card *, unsigned> &num_cards) {
...
unsigned get_deck_cost(const Deck *deck) {
  ...
  if (deck->commander) {
    deck_cost +=
        get_required_cards_before_upgrade({deck->commander}, num_in_deck);
  }
...
bool is_owned_or_can_be_fused(const Card *card) {
  ...
  for (auto it : num_in_deck) {
    if (it.second > owned_cards[it.first->m_id]) {
      if ((card->m_category == CardCategory::dominion_alpha) &&
          use_maxed_dominions && !is_in_recipe(card, owned_alpha_dominion)) {
  ...
  if (!num_under.empty()) {
    std::map<const Card *, unsigned> &refund =
        dominion_refund[owned_alpha_dominion->m_fusion_level]
                       [owned_alpha_dominion->m_level];
...
std::string alpha_dominion_cost(const Card *dom_card) {
  ...
  if (!is_in_recipe(dom_card, owned_alpha_dominion)) {
    num_cards.clear();
    get_required_cards_before_upgrade(_owned_cards, {owned_alpha_dominion},
                                      num_cards);
    value += "using refund: ";
    for (auto &it : refund) {
      signed num_under(it.second - (signed)num_cards[it.first]);
      value += it.first->m_name + " x " + std::to_string(it.second) + "/" +
               std::to_string(num_under) + ", ";
    }
  }
...
// check if claim_cards is necessary => i.e. can the deck be build from the
// ownedcards
...
void claim_cards(const std::vector<const Card *> &card_list) {
  ...
  for (const auto &it : num_cards) {
    ...
    if (num_to_claim > 0) {
      ...
      if (debug_print >= 0) {
        std::cerr
            << "WARNING: Need extra " << num_to_claim << " " << card->m_name
            << " to build your initial deck: adding to owned card list.\n";
      }
    }
  }
}
...
bool valid_deck(Deck *your_deck) {
  ...
  if (your_deck->alpha_dominion &&
      claim_cards_needed({your_deck->alpha_dominion}))
    return false;
  return true; // valid
}
//------------------------------------------------------------------------------
...
void SimulationData::set_decks(std::vector<Deck *> const your_decks_,
                               std::vector<Deck *> const &enemy_decks_) {
...
inline std::vector<Results<uint64_t>> SimulationData::evaluate() {
  ...
  for (Hand *your_hand : your_hands) {
    for (Hand *enemy_hand : enemy_hands) {
      ...
      Field fd(re, cards, *your_hand, *enemy_hand, gamemode, optimization_mode,
               ...
               quest,
#endif
               your_bg_effects, enemy_bg_effects, your_bg_skills,
               enemy_bg_skills, fixes, flexible_iter, flexible_turn, eval_iter,
               eval_turn);
  ...
  // std::cout << std::endl<<  "Deck hash: " << your_hand.deck->hash() << "#"<<
  // std::endl;
  return (res);
}
//------------------------------------------------------------------------------
...
#pragma omp parallel default(none)                                             \
    shared(evaluated_results, thread_num_iterations, results)
  {
...
void Process::openmp_compare_reduction(EvaluatedResults &evaluated_results) {
  ...
  const long double max_possible =
      max_possible_score[(size_t)optimization_mode];
...
#pragma omp declare reduction \
			(VecPlus:std::vector<Results<uint64_t>>: omp_out=merge(omp_out,omp_in))
#pragma omp parallel default(none)                                             \
    shared(evaluated_results, trials, successes, locks, compare_stop,          \
           thread_num_iterations, results, thread_best_results,                \
           min_increment_of_score)
  {
    SimulationData *sim = threads_data.at(omp_get_thread_num());
    sim->set_decks(this->your_decks, this->enemy_decks);
    const long double sim_accum =
        std::accumulate(sim->factors.begin(), sim->factors.end(), .0);
#pragma omp for reduction(VecPlus : results) schedule(runtime)
    for (unsigned i = 0; i < thread_num_iterations; ++i) {
      ...
      if (!compare_stop) {
        ...
        if (omp_get_thread_num() == 0) {
          ...
          if (ssuccesses > ttrials) {
            ssuccesses = ttrials;
            printf(
                "WARNING: biominal successes {%Le} > trials {%d} in Threads\n",
                ssuccesses, ttrials);
            //_DEBUG_MSG(2,"WARNING: biominal successes > trials in Threads");
          }
          compare_stop =
              (boost::math::binomial_distribution<>::find_upper_bound_on_p(
                   ttrials, ssuccesses, prob) *
                   max_possible <
...
std::vector<Results<uint64_t>> merge(std::vector<Results<uint64_t>> out,
                                     std::vector<Results<uint64_t>> in) {
...
#endif
EvaluatedResults &Process::evaluate(unsigned num_iterations,
...
EvaluatedResults &
Process::compare(unsigned num_iterations, EvaluatedResults &evaluated_results,
                 const FinalResults<long double> &best_results) {
  if (num_iterations <= evaluated_results.second) {
    return evaluated_results;
  }
  thread_num_iterations = num_iterations - evaluated_results.second;
...
//------------------------------------------------------------------------------
void thread_evaluate(boost::barrier &main_barrier, boost::mutex &shared_mutex,
                     SimulationData &sim, const Process &p,
                     unsigned thread_id) {
#ifndef _OPENMP
  while (true) {
    main_barrier.wait();
    ...
    while (true) {
      shared_mutex.lock(); //<<<<
      if (thread_num_iterations == 0 ||
          (thread_compare && thread_compare_stop)) //!
      {
        shared_mutex.unlock(); //>>>>
        main_barrier.wait();
        break;
        ...
        shared_mutex.lock(); //<<<<
        std::vector<uint64_t> thread_score_local(thread_results->first.size(),
                                                 0u); //!
        ...
        if (thread_compare && thread_id == 0 && thread_total_local > 1) {
          ...
          // Multiple defense decks case: scaling by factors and approximation
          // of a "discrete" number of events.
          if (result.size() > 1) {
            long double score_accum_d = 0.0;
...
//------------------------------------------------------------------------------
void print_score_info(const EvaluatedResults &results,
                      std::vector<long double> &factors) {
...
//------------------------------------------------------------------------------
void print_results(const EvaluatedResults &results,
                   std::vector<long double> &factors) {
  auto final = compute_score(results, factors);
  ...
  if (optimization_mode == OptimizationMode::quest) {
    // points = win% * win_score + (must_win ? win% : 100%) * quest% *
    // quest_score quest% = (points - win% * win_score) / (must_win ? win% :
    // 100%) / quest_score
    std::cout << "quest%: "
              << (final.points - final.wins * quest.win_score) /
                     (quest.must_win ? final.wins : 1) / quest.quest_score * 100
              << std::endl;
  }
#endif
  ...
  case OptimizationMode::quest:
    ...
    if (optimization_mode == OptimizationMode::brawl) {
      auto win_points =
          final.wins
              ? ((final.points - min_score * (1.0 - final.wins)) / final.wins)
              : final.points;
      std::cout << " [" << win_points << " per win]";
    } else if (optimization_mode == OptimizationMode::brawl_defense) {
      auto opp_win_points =
          final.losses
              ? max_score - ((final.points -
                              (max_score - min_score) * (1.0 - final.losses)) /
                             final.losses)
              : final.points;
    ...
    std::cout << "/ " << results.second << ")" << std::endl;
    if (show_ci) {
      std::cout << "ci: " << final.points_lower_bound << " - "
                << final.points_upper_bound << std::endl;
    }
    break;
  default:
    break;
...
void print_score_inline(const FinalResults<long double> score) {
  ...
  case OptimizationMode::quest:
    ...
    if (optimization_mode == OptimizationMode::quest) {
      std::cout << ", "
                << (score.points - score.wins * quest.win_score) /
                       (quest.must_win ? score.wins : 1) / quest.quest_score *
                       100
                << "% quest";
    }
#endif
    if (show_ci) {
      std::cout << ", " << score.points_lower_bound << "-"
                << score.points_upper_bound;
    }
    std::cout << ") ";
    break;
  ...
  if (optimization_mode == OptimizationMode::brawl) {
    auto win_points =
        score.wins
            ? ((score.points - min_score * (1.0 - score.wins)) / score.wins)
            : score.points;
    std::cout << " [" << win_points << " per win]";
  } else if (optimization_mode == OptimizationMode::brawl_defense) {
    auto opp_win_points =
        score.losses ? max_score - ((score.points - (max_score - min_score) *
                                                        (1.0 - score.losses)) /
                                    score.losses)
                     : score.points;
    std::cout << " [" << opp_win_points << " per opp win]";
...
void print_sim_card_values(Deck *original_deck, Process &p,
                           unsigned iter) // run_deck == p.your_decks[0]
{
  ...
  if (original_deck->strategy == DeckStrategy::random ||
      original_deck->strategy == DeckStrategy::flexible ||
      original_deck->strategy == DeckStrategy::evaluate ||
      original_deck->strategy == DeckStrategy::evaluate_twice) {
    std::sort(original_deck->cards.begin(), original_deck->cards.end(),
              [](const Card *a, const Card *b) { return a->m_id < b->m_id; });
  }
  std::string last_name;
  long double score;
  ...
  EvaluatedResults results = {
      EvaluatedResults::first_type(p.enemy_decks.size() * p.your_decks.size()),
      0};
  ...
  for (unsigned i = 0; i < cards.size(); ++i) {
    ...
    EvaluatedResults results = {EvaluatedResults::first_type(
                                    p.enemy_decks.size() * p.your_decks.size()),
                                0};
    results = p.evaluate(iter, results);
  ...
  if (vc_x > 0) {
    ...
    for (unsigned i = 0; i < cards.size(); ++i) {
      ...
      if (vc_x >= std::distance(msn.find(mns.find(cards[i]->m_name)->second),
                                msn.end())) // or mns.start()
      {
  ...
  if (print_values) {
    ...
    for (unsigned i = 0; i < cards.size(); ++i) {
      std::cout << cards[i]->m_name << " ("
                << mns.find(cards[i]->m_name)->second << "), ";
    }
    std::cout << std::endl;
  }
}
//------------------------------------------------------------------------------
void print_deck_inline(const unsigned deck_cost,
                       const FinalResults<long double> score, Deck *deck,
                       bool print_locked) {
  // print units count
...
if (optimization_mode == OptimizationMode::quest)
{
std::cout << ", " << (score.points - score.wins * quest.win_score) /
(quest.must_win ? score.wins : 1) / quest.quest_score * 100 << "% quest";
}
#endif
if (show_ci)
{
...
if (optimization_mode == OptimizationMode::brawl)
{
auto win_points = score.wins ? ((score.points - min_score * (1.0 - score.wins))
/ score.wins) : score.points; std::cout << " [" << win_points << " per win]";
}
else if (optimization_mode == OptimizationMode::brawl_defense)
{
auto opp_win_points = score.losses ? max_score - ((score.points - (max_score -
min_score) * (1.0 - score.losses)) / score.losses) : score.points; std::cout <<
" [" << opp_win_points << " per opp win]";
}
else if (optimization_mode == OptimizationMode::war)
{
auto win_points = score.wins ? ((score.points - min_score * (1.0 - score.wins))
...
*/
  ...
  if (deck->strategy == DeckStrategy::random ||
      deck->strategy == DeckStrategy::flexible ||
      deck->strategy == DeckStrategy::evaluate ||
      deck->strategy == DeckStrategy::evaluate_twice) {
...
std::vector<std::vector<const Card *>> get_candidate_lists(Process &proc) {
  ...
  player_assaults_and_structures.insert(player_assaults_and_structures.end(),
                                        proc.cards.player_structures.begin(),
                                        proc.cards.player_structures.end());
  player_assaults_and_structures.insert(player_assaults_and_structures.end(),
                                        proc.cards.player_assaults.begin(),
                                        proc.cards.player_assaults.end());
  for (auto it = player_assaults_and_structures.begin();
       it != player_assaults_and_structures.end(); ++it) {
    const Card *card = *it;
    // skip illegal
    if ((card->m_category != CardCategory::dominion_alpha) &&
    ...
    if (!allowed_candidates.count(card->m_id)) {
      ...
      if (card->m_category == CardCategory::dominion_alpha) {
        ...
        // (will check it later and downgrade if necessary according to amount
        // of material (shards))
      ...
      // handle normal cards
        ...
        // skip non-top-level cards (adjust_deck() will try to downgrade them if
        // necessary)
        bool use_top_level = (card->m_type == CardType::commander)
                                 ? use_top_level_commander
                                 : use_top_level_card;
        if (!card->is_top_level_card() and
            (fund || use_top_level || !owned_cards[card->m_id])) {
          continue;
        }

        // skip lowest fusion levels
        unsigned use_fused_level = (card->m_type == CardType::commander)
                                       ? use_fused_commander_level
                                       : use_fused_card_level;
    ...
    if (use_owned_cards && card->m_category == CardCategory::dominion_alpha &&
        !owned_cards[card->m_id]) {
    ...
    if (use_owned_cards &&
        ...
        !is_owned_or_can_be_fused(card)) {
      continue;
    }
    // mono
    if (!factions.empty() &&
        (std::find(factions.begin(), factions.end(), card->m_faction) ==
         factions.end()) != invert_factions) // XOR
    {
      continue;
    }
    ...
    if (card->m_type == CardType::commander) {
      ...
      if (only_recent &&
          !((it + (player_assaults_and_structures.size() * recent_percent /
                   100)) > player_assaults_and_structures.end()))
        continue; // latest %
      if (prefered_recent &&
          it + (player_assaults_and_structures.size() * recent_percent) / 100 >
              player_assaults_and_structures.end()) // latest %
      {
...
//------------------------------------------------------------------------------
extern void (*skill_table[Skill::num_skills])(Field *, CardStatus *src_status,
                                              const SkillSpec &);
...
bool check_input_amount(int argc, const char **argv, int argIndex, int number) {
  if (argc <= argIndex + number) {
    std::cerr << argv[argIndex] << " needs " << number << " paramters"
              << std::endl;
    return true;
  }
  return false;
...
void usage(int argc, const char **argv) {
  std::cout
      << "Tyrant Unleashed Optimizer (TUO) " << TYRANT_OPTIMIZER_VERSION
      << "\n"
         "usage: "
      << argv[0]
      << " Your_Deck Enemy_Deck [Flags] [Operations]\n"
         "\n"
         "Your_Deck:\n"
         ...
         "  where deck is the name/hash/cards of a mission, raid, quest or "
         "custom deck, and factor is optional. The default factor is 1.\n"
         "  example: \'fear:0.2;slowroll:0.8\' means fear is the defense deck "
         "20% of the time, while slowroll is the defense deck 80% of the "
         "time.\n"
         "\n"
         "Flags:\n"
         "  -e \"<effect>\": set the battleground effect; you may use -e "
         "multiple times.\n"
         "  -r: the attack deck is played in order instead of randomly "
         "(respects the 3 cards drawn limit).\n"
         ...
         "  defense: simulate/optimize for win rate + stall rate. can be used "
         "for defending deck or win rate oriented raid simulations.\n"
         "  raid:    simulate/optimize for average raid damage (ARD). default "
         "for raids.\n"
         "Flags for climb:\n"
         ...
         "  climb <num>: perform hill-climbing starting from the given attack "
         "deck, using up to <num> battles to evaluate a deck.\n"
         "  reorder <num>: optimize the order for given attack deck, using up "
         "to <num> battles to evaluate an order.\n"
#ifndef NDEBUG
         "  debug: testing purpose only. very verbose output. only one "
         "battle.\n"
         "  debuguntil <min> <max>: testing purpose only. fight until the last "
         "fight results in range [<min>, <max>]. recommend to redirect "
         "std::coutput.\n"
#endif
      ;
}
...
bool parse_bge(
    std::string bge_name, unsigned player,
  ...
  if (bge_name.find_first_of(";|") != std::string::npos) {
    ...
    for (auto &&next_bge : bges) {
      if (!parse_bge(next_bge, player, bge_aliases, your_bg_effects,
                     enemy_bg_effects, your_bg_skills, enemy_bg_skills,
                     used_bge_aliases)) {
        return false;
      }
    }
  ...
  if (bge_alias_itr != bge_aliases.end()) {
    if (!used_bge_aliases.insert(simple_bge_name).second) {
      throw std::runtime_error("BGE alias: " + bge_name +
                               ": Circular reference");
    }
    return parse_bge(bge_alias_itr->second, player, bge_aliases,
                     your_bg_effects, enemy_bg_effects, your_bg_skills,
                     enemy_bg_skills, used_bge_aliases);
  }
  ...
  try {
    for (auto &&skill_name : skill_name_list) {
      PassiveBGE::PassiveBGE passive_bge_id =
      ...
      if (passive_bge_id != PassiveBGE::no_bge) {
        ...
        signed short bge_value =
            (tokens.size() > 1) ? boost::lexical_cast<signed short>(tokens[1])
                                : 1;
        if (!bge_value)
          throw std::runtime_error("BGE " + skill_name +
                                   ": zero value means no BGE");
        ...
        // activation BG skill
        SkillSpec bg_skill{
            skill_id,        0,     allfactions, 0, 0, Skill::no_skill,
            Skill::no_skill, false, 0,
        };

        // skill [ ALL | N ] ...
        if (skill_index < tokens.size() &&
            boost::to_lower_copy(tokens[skill_index]) == "all") {
          bg_skill.all = true;
          skill_index += 1;
        } else if (skill_index + 1 < tokens.size() &&
                   isdigit(*tokens[skill_index].c_str())) {
        ...
        if (skill_index < tokens.size()) {
          ...
          if (bg_skill.s != Skill::no_skill) {
            skill_index += 1;
            if (skill_index < tokens.size() &&
                (boost::to_lower_copy(tokens[skill_index]) == "to" ||
                 boost::to_lower_copy(tokens[skill_index]) == "into")) {
              skill_index += 1;
            }
            if (skill_index < tokens.size()) {
...
DeckResults run(int argc, const char **argv) {
  ...
  for (int argIndex = 3; argIndex < argc; ++argIndex) {
    ...
    // Others
    else if (strcmp(argv[argIndex], "keep-commander") == 0 ||
             strcmp(argv[argIndex], "-c") == 0) {
      opt_keep_commander = true;
    } else if (strcmp(argv[argIndex], "mono") == 0 ||
               strcmp(argv[argIndex], "-m") == 0 ||
               strcmp(argv[argIndex], "factions") == 0 ||
               strcmp(argv[argIndex], "-f") == 0) {
      ...
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "no-mono") == 0 ||
               strcmp(argv[argIndex], "no-factions") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      factions.push_back(faction_name_to_id(argv[argIndex + 1]));
      invert_factions = true;
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "strategy") == 0 ||
               strcmp(argv[argIndex], "skill") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      if (strcmp(argv[argIndex + 1], "") != 0) {
      ...
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "no-strategy") == 0 ||
               strcmp(argv[argIndex], "no-skill") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      if (strcmp(argv[argIndex + 1], "") != 0) {
        skills.push_back(skill_name_to_id(argv[argIndex + 1]));
      ...
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "prefered-strategy") == 0 ||
               strcmp(argv[argIndex], "prefered-skill") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      if (strcmp(argv[argIndex + 1], "") != 0) {
        if (strcmp(argv[argIndex + 1], "recent") == 0) {
      ...
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "effect") == 0 ||
               strcmp(argv[argIndex], "-e") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      opt_effects[2].push_back(argv[argIndex + 1]);
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "ye") == 0 ||
               strcmp(argv[argIndex], "yeffect") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      opt_effects[0].push_back(argv[argIndex + 1]);
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "ee") == 0 ||
               strcmp(argv[argIndex], "eeffect") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      opt_effects[1].push_back(argv[argIndex + 1]);
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "freeze") == 0 ||
               strcmp(argv[argIndex], "-F") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      ...
      use_maxed_dominions = false;
    } else if (strcmp(argv[argIndex], "dom+") == 0 ||
               strcmp(argv[argIndex], "dominion+") == 0 ||
               strcmp(argv[argIndex], "dom-owned") == 0) {
      use_owned_dominions = true;
      use_maxed_dominions = false;
    } else if (strcmp(argv[argIndex], "dom-") == 0 ||
               strcmp(argv[argIndex], "dominion-") == 0 ||
               strcmp(argv[argIndex], "dom-maxed") == 0) {
      use_owned_dominions = true;
      ...
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "evaluate") == 0 ||
               strcmp(argv[argIndex], "eval") == 0) {
      opt_your_strategy = DeckStrategy::evaluate;
    } else if (strcmp(argv[argIndex], "evaluate2") == 0 ||
               strcmp(argv[argIndex], "eval2") == 0) {
      ...
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "timeout") ==
               0) // set timeout in hours. tuo will stop approx. at the given
                  // time.
    {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      ...
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "strict-ownedcards") ==
               0) // won't add missing cards, instead skips faulty decks
    {
      opt_skip_unclaimed_decks = true;
    }
    /*else if (strcmp(argv[argIndex], "trim-ownedcards") == 0) //won't add
      missing cards, instead removes missing cards from decks
      {
    ...
    } else if (strcmp(argv[argIndex], "hand") == 0) // set initial hand for test
    {
      ...
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "enemy:hand") ==
               0) // set enemies' initial hand for test
    {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      ...
      argIndex += 1;
               ...
               strcmp(argv[argIndex], "yfort") == 0) // set forts
    {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      opt_forts = std::string(argv[argIndex + 1]);
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "yfpool") == 0 ||
               strcmp(argv[argIndex], "yfortpool") == 0) // set forts
    {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      ...
      argIndex += 1;
               ...
               strcmp(argv[argIndex], "efort") == 0) // set enemies' forts
    {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      opt_enemy_forts = std::string(argv[argIndex + 1]);
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "efpool") == 0 ||
               strcmp(argv[argIndex], "efortpool") == 0) // set forts
    {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      efpool = std::stoi(argv[argIndex + 1]);
      argIndex += 1;
    } else if (strcmp(argv[argIndex], "yd") == 0 ||
               strcmp(argv[argIndex], "ydom") == 0) // set dominions
    {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      ...
      argIndex += 1;
               ...
               strcmp(argv[argIndex], "edom") == 0) // set enemies' dominions
    {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      ...
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      opt_todo.push_back(
          std::make_tuple((unsigned)atoi(argv[argIndex + 1]), 0u, simulate));
    ...
    // climbing tasks
      ...
      if (check_input_amount(argc, argv, argIndex, 2))
        exit(1);
      opt_todo.push_back(std::make_tuple((unsigned)atoi(argv[argIndex + 1]),
                                         (unsigned)atoi(argv[argIndex + 2]),
                                         climb));
      ...
      argIndex += 1;
               ...
               strcmp(argv[argIndex], "beam-size") == 0 ||
               strcmp(argv[argIndex], "size") == 0) {
      if (check_input_amount(argc, argv, argIndex, 1))
        exit(1);
      ...
      boost::tokenizer<boost::char_delimiters_separator<char>> climb_opts{
          climb_opts_str,
          boost::char_delimiters_separator<char>{false, ",", ""}};
      for (const auto &opt : climb_opts) {
        ...
        if (slash_pos == std::string::npos)
          throw std::runtime_error("scale-opts:" + opt +
                                   " requires an argument");
        const std::string &opt_type = has_value ? opt.substr(0, dot_pos) : "";
        const std::string opt_name{
            has_value ? opt.substr(dot_pos + 1, slash_pos - dot_pos - 1)
                      : opt.substr(0, slash_pos)};
        ...
        if ((opt_name == "hp")) {
          ...
          std::cerr << "Error: Unknown scale option " << opt_name << " of "
                    << opt_type;
          if (has_value) {
    ...
    // climbing options
      ...
      for (const auto &opt : climb_opts) {
        ...
        const std::string &opt_name =
            has_value ? opt.substr(0, delim_pos) : opt;
        const std::string opt_value{has_value ? opt.substr(delim_pos + 1)
                                              : opt};
        auto ensure_opt_value = [](const bool has_value,
                                   const std::string &opt_name) {
          if (!has_value) {
            throw std::runtime_error("climb-opts:" + opt_name +
                                     " requires an argument");
          }
        };
        ...
        } else if ((opt_name == "egc") or (opt_name == "endgame-commander") or
                   (opt_name == "min-commander-fusion-level")) {
          ensure_opt_value(has_value, opt_name);
        ...
        } else if ((opt_name == "recent-boost") or
                   (opt_name == "rb")) // prefer new cards in hill climb and
                                       // break climb loop faster
        {
          prefered_recent = true;
        } else if ((opt_name == "recent-boost-times") or
                   (opt_name == "rbt")) // prefer new cards in hill climb and
                                        // break climb loop faster
        {
          ensure_opt_value(has_value, opt_name);
          prefered_factor = std::stoi(opt_value);
        } else if ((opt_name == "recent-boost-percent") or
                   (opt_name == "rbp")) // prefer new cards in hill climb and
                                        // break climb loop faster
        {
          ensure_opt_value(has_value, opt_name);
      ...
      // output the debug info for the first battle that min_score <= score <=
      // max_score. E.g., 0 0: lose; 100 100: win (non-raid); 20 100: at least
      // 20 damage (raid).
      if (check_input_amount(argc, argv, argIndex, 2))
        exit(1);
      opt_todo.push_back(std::make_tuple((unsigned)atoi(argv[argIndex + 1]),
  ...
  // if(all_cards.all_cards.size()>0) delete(&all_cards); // complains invalid
  // pointer all_cards.organize();
  all_cards = Cards();
  Decks decks;
  std::unordered_map<std::string, std::string> bge_aliases;
  load_skills_set_xml(all_cards, prefix + "data/skills_set.xml", true);
  for (unsigned section = 1; load_cards_xml(
           all_cards,
           prefix + "data/cards_section_" + to_string(section) + ".xml", false);
       ++section)
    ;
  all_cards.organize();
  ...
  for (const auto &suffix : fn_suffix_list) {
    load_decks_xml(decks, all_cards, prefix + "data/missions" + suffix + ".xml",
                   prefix + "data/raids" + suffix + ".xml", suffix.empty());
    load_recipes_xml(all_cards,
                     prefix + "data/fusion_recipes_cj2" + suffix + ".xml",
                     suffix.empty());
    read_card_abbrs(all_cards, prefix + "data/cardabbrs" + suffix + ".txt");
  }
  for (const auto &suffix : fn_suffix_list) {
    load_custom_decks(decks, all_cards,
                      prefix + "data/customdecks" + suffix + ".txt");
    map_keys_to_set(
        read_custom_cards(all_cards,
                          prefix + "data/allowed_candidates" + suffix + ".txt",
                          false),
        allowed_candidates);
    map_keys_to_set(read_custom_cards(all_cards,
                                      prefix + "data/disallowed_candidates" +
                                          suffix + ".txt",
                                      false),
                    disallowed_candidates);
    map_keys_to_set(
        read_custom_cards(all_cards,
                          prefix + "data/disallowed_recipes" + suffix + ".txt",
                          false),
        disallowed_recipes);
  }
  ...
  if (opt_do_optimization and use_owned_cards) {
    if (opt_owned_cards_str_list
            .empty()) { // load default files only if specify no -o=
      for (const auto &suffix : fn_suffix_list) {
    ...
    for (auto owned_it = _owned_cards.begin();
         owned_it != _owned_cards.end();) {
      const Card *owned_card = all_cards.by_id(owned_it->first);
      bool need_remove = (!owned_it->second);
      if (!need_remove &&
          (owned_card->m_category == CardCategory::dominion_alpha)) {
        if (!owned_alpha_dominion) {
          owned_alpha_dominion = owned_card;
        } else {
          /*std::cerr << "Warning: ownedcards already contains alpha dominion ("
            << owned_alpha_dominion->m_name
    ...
    if (!owned_alpha_dominion && use_owned_dominions) {
      ...
      // std::cerr << "Warning: dominion climbing enabled and no alpha dominion
      // found in owned cards, adding default "
  ...
  for (int player = 2; player >= 0; --player) {
    for (auto &&opt_effect : opt_effects[player]) {
      std::unordered_set<std::string> used_bge_aliases;
      if (!parse_bge(opt_effect, player, bge_aliases, opt_bg_effects[0],
                     opt_bg_effects[1], opt_bg_skills[0], opt_bg_skills[1],
                     used_bge_aliases)) {
        exit(1);
      }
    }
  ...
  try {
    auto &&id_marks =
        string_to_ids(all_cards, opt_allow_candidates, "allowed-candidates");
    for (const auto &cid : id_marks.first) {
      allowed_candidates.insert(cid);
    }
  } catch (const std::runtime_error &e) {
    std::cerr << "Error: allow-candidates " << opt_allow_candidates << ": "
              << e.what() << std::endl;
    exit(1);
  }
  ...
  try {
    auto &&id_marks = string_to_ids(all_cards, opt_disallow_candidates,
                                    "disallowed-candidates");
    for (const auto &cid : id_marks.first) {
      disallowed_candidates.insert(cid);
    }
  } catch (const std::runtime_error &e) {
    std::cerr << "Error: disallow-candidates " << opt_disallow_candidates
              << ": " << e.what() << std::endl;
    exit(1);
  }
  ...
  try {
    auto &&id_dis_recipes =
        string_to_ids(all_cards, opt_disallow_recipes, "disallowed-recipes");
    for (auto &cid : id_dis_recipes.first) {
      all_cards.erase_fusion_recipe(cid);
    }
  } catch (const std::runtime_error &e) {
    std::cerr << "Error: disallow-recipes " << opt_disallow_recipes << ": "
              << e.what() << std::endl;
    exit(1);
  }
  ...
  if (!opt_quest.empty()) {
    try {
      ...
      if (tokens.size() < 3) {
        throw std::runtime_error("Expect one of: su n skill; sd n skill; cu n "
                                 "faction/strcture; ck n structure");
      }
      ...
      if (type_str == "su" || type_str == "sd") {
        ...
        quest.quest_type =
            type_str == "su" ? QuestType::skill_use : QuestType::skill_damage;
        quest.quest_key = skill_id;
      } else if (type_str == "cu" || type_str == "ck") {
        if (key_str == "assault") {
          quest.quest_type = type_str == "cu" ? QuestType::type_card_use
                                              : QuestType::type_card_kill;
          ...
          for (unsigned i = 1; i < Faction::num_factions; ++i) {
            if (key_str == boost::to_lower_copy(faction_names[i])) {
              quest.quest_type = type_str == "cu"
                                     ? QuestType::faction_assault_card_use
                                     : QuestType::faction_assault_card_kill;
              quest.quest_key = i;
              break;
            }
          }
          if (quest.quest_key == 0) {
            std::cerr
                << "Error: Expect assault, structure or faction in quest \""
                << opt_quest << "\".\n";
            exit(1);
          }
        }
        ...
        char mark;
        try {
          parse_card_spec(all_cards, key_str, card_id, card_num, num_sign,
                          mark);
        ...
        char mark;
        try {
          parse_card_spec(all_cards, boost::to_lower_copy(tokens[3]), card_id,
                          card_num, num_sign, mark);
          quest_index += 1;
      ...
      max_possible_score[(size_t)optimization_mode] =
          quest.quest_score + quest.win_score;
    } catch (const boost::bad_lexical_cast &e) {
      std::cerr << "Error: Expect a number in quest \"" << opt_quest << "\".\n";
      exit(1);
    } catch (const std::runtime_error &e) {
      std::cerr << "Error: quest " << opt_quest << ": " << e.what()
                << std::endl;
      exit(1);
    }
  }
  ...
  for (auto deck_parsed : your_deck_list_parsed) {
    ...
    try {
      ...
      std::cerr << "Error: Deck " << deck_parsed.first << ": " << e.what()
                << std::endl;
      exit(1);
    }
    if (your_deck == nullptr) {
      ...
      std::cerr << "Error: Invalid attack deck " << deck_parsed.first
                << ": has optional cards.\n";
      your_deck = nullptr;
    ...
    if (!opt_doms.empty()) {
      try {
        ...
        std::cerr << "Error: ydom " << opt_doms << ": " << e.what()
                  << std::endl;
        exit(1);
      }
    }
    ...
    if (opt_do_optimization || opt_do_reorder) {
      ...
      for (auto &&card_mark : your_deck->card_marks) {
        auto &&card = card_mark.first < 0 ? your_deck->commander
                                          : your_deck->cards[card_mark.first];
      ...
      // NOTE: do this AFTER the call to claim_cards so that passing an initial
      // deck of >10 cards
      //       can be used as a "shortcut" for adding them to owned cards. Also
      //       this allows climb to figure out which are the best 10, rather
      //       than restricting climb to the first 10.
  ...
  if (your_decks.size() == 0) {
    std::cerr << "No deck set. Probably due to 'strict-ownedcards' and a deck "
                 "that is incompatible to the set 'ownedcards'."
              << std::endl;
    exit(1);
  }
  target_score = opt_target_score.empty()
                     ? max_possible_score[(size_t)optimization_mode]
                     : boost::lexical_cast<long double>(opt_target_score);
  ...
  for (auto deck_parsed : enemy_deck_list_parsed) {
    ...
    if (!opt_enemy_doms.empty()) {
      try {
        ...
        std::cerr << "Error: edom " << opt_enemy_doms << ": " << e.what()
                  << std::endl;
        exit(1);
      }
    }
    ...
    try {
      ...
      std::cerr << "Error: enemy:hand " << opt_enemy_hand << ": " << e.what()
                << std::endl;
      exit(1);
    }
  ...
  std::vector<long double> factors(
      (opt_multi_optimization ? 1 : your_decks_factors.size()) *
      enemy_decks_factors.size());
  for (unsigned i = 0; i < factors.size(); ++i) {
    factors[i] = your_decks_factors[i / enemy_decks_factors.size()] *
                 enemy_decks_factors[i % enemy_decks_factors.size()];
  }

  if ((opt_do_optimization || opt_do_reorder) &&
      (your_decks.size() != 1 && !opt_multi_optimization)) {
  ...
  if (debug_print >= 0) {
    for (unsigned i(0); i < your_decks.size(); ++i) {
      ...
      std::cout << "Your Deck:" << your_decks_factors[i] << ": "
                << (debug_print > 0 ? your_deck->long_description()
                                    : your_deck->medium_description())
                << std::endl;
    }
    for (unsigned bg_effect = PassiveBGE::no_bge;
         bg_effect < PassiveBGE::num_passive_bges; ++bg_effect) {
    ...
    for (const auto &bg_skill : opt_bg_skills[0]) {
      std::cout << "Your BG Skill: " << skill_description(all_cards, bg_skill)
                << std::endl;
    }
    ...
    for (unsigned i(0); i < enemy_decks.size(); ++i) {
      ...
      std::cout << "Enemy's Deck:" << enemy_decks_factors[i] << ": "
                << (debug_print > 0 ? enemy_deck->long_description()
                                    : enemy_deck->medium_description())
                << std::endl;
    }
    for (unsigned bg_effect = PassiveBGE::no_bge;
         bg_effect < PassiveBGE::num_passive_bges; ++bg_effect) {
    ...
    for (const auto &bg_skill : opt_bg_skills[1]) {
      std::cout << "Enemy's BG Skill: "
                << skill_description(all_cards, bg_skill) << std::endl;
    }
  }
  if (enemy_decks.size() == 1) {
    ...
    for (auto x_mult_ss : enemy_deck->effects) {
      if (debug_print >= 0) {
        std::cout << "Enemy's X-Mult BG Skill (effective X = round_up[X * "
                  << enemy_deck->level
      ...
      opt_bg_skills[1].push_back(
          {x_mult_ss.id, (unsigned)ceil(x_mult_ss.x * enemy_deck->level),
           x_mult_ss.y, x_mult_ss.n, x_mult_ss.c, x_mult_ss.s, x_mult_ss.s2,
           x_mult_ss.all});
    }
  }
  auto proc_decks = your_decks;
  if (opt_multi_optimization) { // only one deck at a time
    proc_decks.erase(proc_decks.begin() + 1, proc_decks.end());
  }
  Process p(opt_num_threads, all_cards, decks, proc_decks, enemy_decks, factors,
            gamemode,
#ifndef NQUEST
            quest,
#endif
            opt_bg_effects[0], opt_bg_effects[1], opt_bg_skills[0],
            opt_bg_skills[1]);
  ...
  // for (auto op: opt_todo)
  {
    ...
    case simulate: {
      EvaluatedResults results = {
          EvaluatedResults::first_type(enemy_decks.size() * your_decks.size()),
          0};
      results = p.evaluate(std::get<0>(op), results);
      print_results(results, p.factors);
      fr =
          std::make_pair(your_deck->clone(), compute_score(results, p.factors));
    ...
    case climb: {
      ...
      fr = hill_climbing(std::get<0>(op), std::get<1>(op), your_decks, p,
                         requirement
#ifndef NQUEST
                         ,
                         quest
    ...
    case anneal: {
      // TODO check for your_decks.size()==1
      fr = simulated_annealing(std::get<0>(op), std::get<1>(op), your_decks, p,
                               requirement
#ifndef NQUEST
                               ,
                               quest
    ...
    case genetic: {
      fr = genetic_algorithm(std::get<0>(op), std::get<1>(op), your_decks, p,
                             requirement
#ifndef NQUEST
                             ,
                             quest
    ...
    case beam: {
      fr = beam_climb(std::get<0>(op), std::get<1>(op), your_decks, p,
                      requirement
#ifndef NQUEST
                      ,
                      quest
    ...
    case reorder: {
      ...
      fr = hill_climbing(std::get<0>(op), std::get<1>(op), single_deck, p,
                         requirement
#ifndef NQUEST
                         ,
                         quest
    ...
    case debug: {
      ++debug_print;
      debug_str.clear();
      EvaluatedResults results{EvaluatedResults::first_type(enemy_decks.size()),
                               0};
    ...
    case debuguntil: {
      ...
      while (true) {
        debug_str.clear();
        EvaluatedResults results{
            EvaluatedResults::first_type(enemy_decks.size()), 0};
        results = p.evaluate(1, results);
        auto score = compute_score(results, p.factors);
        if (score.points >= std::get<0>(op) &&
            score.points <= std::get<1>(op)) {
            ...
            if (first_line == "") {
              ...
              while (ss >> boost::io::quoted(tmp)) {
                for (unsigned z = 0; z < result_decks.size(); ++z) {
                  boost::replace_all(tmp, "@" + std::to_string(z) + "@",
                                     result_decks[z]);
                }
                for (int z = 0; z < argc - j - 1; ++z) {
                  std::string rep("$" + std::to_string(z) + "$");
              ...
              if (!first) {
                ...
                cur_split.insert(cur_split.begin() + 1,
                                 result_decks[result_decks.size() - 1]);
              }
              first = false;
...
int main(int argc, const char **argv) {
  ...
  if (argc == 2 && strcmp(argv[1], "-version") == 0) {
    std::cout << "Tyrant Unleashed Optimizer " << TYRANT_OPTIMIZER_VERSION
              << std::endl;
    return 0;
  }
  if (argc < 2) {
